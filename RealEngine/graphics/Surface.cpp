...
namespace RE {
	...
	void Surface::clear(const glm::vec4& colour, int index) {
	...
	void SurfaceTargetTextures::targetTexture(unsigned int index) {
		assert(index < sizeof(m_buffers) / sizeof(m_buffers[0]));
		m_buffers[index] = GL_COLOR_ATTACHMENT0 + index;
	}
