...
#include <RealEngine/GeometryBatch.hpp>
...
namespace RE {
	...
	void MainProgram::E_SDL(SDL_Event& evnt) {
		...
		case SDL_KEYUP:
			if (evnt.key.repeat == 0) {
				p_inputManager.press(RE::RKey::ANY_KEY, -1);
				p_inputManager.release(SDLKToRkey(evnt.key.keysym.sym));
			}
			break;
		case SDL_MOUSEBUTTONDOWN:
			if (evnt.key.repeat == 0) {
				auto key = SDLKToRkey(evnt.button.button);
				p_inputManager.press(RE::RKey::ANY_KEY);
		...
		case SDL_MOUSEBUTTONUP:
			if (evnt.key.repeat == 0) {
				p_inputManager.press(RE::RKey::ANY_KEY, -1);
				p_inputManager.release(SDLKToRkey(evnt.button.button));
			}
			break;
	...
	bool MainProgram::initSystems() {
		...
		//Standard shaders
		m_stdSpriteShader = RE::RM::getShaderProgram(ShaderProgramSeed::stdSprite);
		m_stdGeometryShader = RE::RM::getShaderProgram(ShaderProgramSeed::stdGeometry);
		//Standard uniforms
