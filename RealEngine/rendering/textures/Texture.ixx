 ...
 *  @author    Dubsky Tomas
 */
export module RealEngine.rendering.textures.Texture;
import std;
...
export import RealEngine.rendering.textures.TextureSeed;
export import RealEngine.rendering.Renderer;
import RealEngine.rendering.textures.PNGLoader;
...
public:
    ...
    */
    void clear(const glm::vec4& color) const { s_impl->clear(m_id, 0, color); }
    void clear(const glm::ivec4& color) const { s_impl->clear(m_id, 0, color); }
    void clear(const glm::uvec4& color) const { s_impl->clear(m_id, 0, color); }
    void clear(Color color) const { s_impl->clear(m_id, 0, color); }
...
Texture<R>::Texture(const std::string& filePathPNG) {
    PNGLoader::LoadedData tex{.params = DEFAULT_PARAMETERS};
    if (PNGLoader::load(filePathPNG, tex) == 0) {
        init(Raster{tex.dims, tex.channels, tex.pixels}, tex.params);
    }
}
