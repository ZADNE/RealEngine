...
Pipeline::Pipeline(const PipelineGraphicsCreateInfo& createInfo, const PipelineGraphicsSources& srcs) {
    ...
    //Create pipeline
    m_pipeline = device().createGraphicsPipeline(pipelineCache(), vk::GraphicsPipelineCreateInfo{{},
...
Pipeline::Pipeline(const PipelineComputeCreateInfo& createInfo, const PipelineComputeSources& srcs) {
    //Create compute shader module
    vk::ShaderModule compShader = device().createShaderModule(
        vk::ShaderModuleCreateInfo{{},
            srcs.comp.vk13.size() * 4,
            srcs.comp.vk13.data()
        }
    );
    //Create pipeline
